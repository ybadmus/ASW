...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    if (businessNews.length === 0) {
      fetchPosts("https://cryptic-ridge-64289.herokuapp.com/api/v1/posts").then(news => {
        setBusinessNews(news);
        setLoading(false);
      });
    }
  ...
  useEffect(() => {
    ...
    if (mixArray.length === 0) {
      fetchData("https://cryptic-ridge-64289.herokuapp.com/api/v1/posts/top_news").then(news => {
        setMixArray(news);
        setLoading(false);
      });
    }
